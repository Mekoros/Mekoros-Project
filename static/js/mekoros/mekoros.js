...
if (typeof window !== 'undefined') {
    window.Mekoros = Mekoros; // allow access to `Mekoros` from console
}

Mekoros = extend(Mekoros, {
  _parseRef: {}, // cache for results of local ref parsing
  parseRef: function(q) {
  // Client side ref parsing without depending on book index data.
  // Does depend on Mekoros.booksDict.
  // One of the oldest functions in Mekoros! But should be intelligently merged into Mekoros.ref()
      q = q || "";
      q = decodeURIComponent(q);
      q = q.replace(/_/g, " ").replace(/[.:]/g, " ").replace(/ +/, " ");
      q = q.trim().toFirstCapital();
      if (q in Mekoros._parseRef) { return Mekoros._parseRef[q]; }
      ...
      for (let i = first.length; i >= 0; i--) {
          ...
          if (Mekoros.virtualBooks.includes(book)) {
          ...
          if (book in Mekoros.booksDict || book === "Sheet") {
      ...
      if (!book) {
          Mekoros._parseRef[q] = {"error": "Unknown book."};
          return Mekoros._parseRef[q];
      }

      if (nums && !nums.match(/\d+[ab]?( \d+)*$/)) {
          Mekoros._parseRef[q] = {"error": "Bad section string."};
          console.log(Mekoros._parseRef[q]);
          return Mekoros._parseRef[q];
      }
  ...
  normRef: function(ref) {
      ...
      if (ref instanceof Array) {
        return Mekoros.normRefList(ref);
      }
      const norm = Mekoros.makeRef(Mekoros.parseRef(ref));
  ...
  humanRef: function(ref) {
      ...
      // `ref` may be a string, or an array of strings. If ref is an array of strings, it is passed to normRefList.
      ref = Mekoros.normRef(ref);
      const pRef = Mekoros.parseRef(ref);
  ...
  isRef: function(ref) {
    // Returns true if `ref` appears to be a ref relative to known books in Mekoros.books
    const q = Mekoros.parseRef(ref);
  ...
  normRefList: function(refs) {
    ...
    const pRef = Mekoros.parseRef(refs[0]);
    const pRefEnd = Mekoros.parseRef(refs[refs.length-1]);
    if (pRef.book !== pRefEnd.book) {
      return refs[0]; // We don't handle ranges over multiple nodes of complex texts
    }
    const nRef = Mekoros.util.clone(pRef);
    nRef.toSections = pRefEnd.toSections;
    return Mekoros.makeRef(nRef);
  },
  joinRefList: function(refs, lang){
    ...
    // Hebrew results depend on `refs` being available in the refs cache.
      ...
      if (refs[0].indexOf("-") != -1) { // did we get a ranged ref for some reason inside the arguemnts
          let startSplit = Mekoros.splitRangingRef(refs[0])
          start = Mekoros.getRefFromCache(startSplit[0]);
      }else{
          start = Mekoros.getRefFromCache(refs[0]);
      }
      if (!start) { // We don't have this ref in cache, fall back to normRefList and sorry no Hebrew
        return Mekoros.humanRef(Mekoros.normRefList(refs));
      }
      start = start[refStrAttr]
      if (refs[refs.length - 1].indexOf("-") != -1) {
          let endSplit = Mekoros.splitRangingRef(refs[refs.length - 1]);
          end = Mekoros.getRefFromCache(endSplit[endSplit.length -1])[refStrAttr];
      }else{
          end = Mekoros.getRefFromCache(refs[refs.length - 1])[refStrAttr];
      }
      ...
      if(namedPartStart != namedPartEnd){
          ...
          const similarpart = Mekoros.util.commonSubstring(start, end);
  ...
  refContains: function(ref1, ref2) {
    ...
    const oRef1 = Mekoros.parseRef(ref1);
    const oRef2 = Mekoros.parseRef(ref2);
    ...
    const [oRef1sections, oRef1toSections, oRef2sections, oRef2toSections] = [oRef1.sections, oRef1.toSections, oRef2.sections, oRef2.toSections].map(arr =>
        arr.map(x => x.match(/\d+[ab]/) ? Mekoros.hebrew.dafToInt(x) : parseInt(x))
    )
  ...
  refCategories: function(ref) {
    ...
    let pRef = Mekoros.parseRef(ref);
    if ("error" in pRef) { return []; }
    let index = Mekoros.index(pRef.index);
  ...
  sectionRef: function(ref, deriveIfNotFound=false) {
    ...
    if (deriveIfNotFound && !oref) { //couldn't find `ref` in cache so try to derive it
        const humanRefForm = Mekoros.humanRef(ref);
  ...
  splitRangingRef: function(ref) {
    ...
    const textData = Mekoros.getTextFromCache(ref);
    if (textData) {
        return Mekoros.makeSegments(textData).map(segment => segment.ref);
      ...
      const newRef = Mekoros.util.clone(oRef);
      newRef.toSections = newRef.sections;
      return Mekoros.splitRangingRef(this.humanRef(this.makeRef(newRef)));
  ...
  getSectionStringByAddressType: function(addressType, i, offset=0) {
    ...
    if (addressType === 'Talmud') {
      enSection = Mekoros.hebrew.intToDaf(section);
      heSection = Mekoros.hebrew.encodeHebrewDaf(enSection);
    } else if (addressType === "Year") {
      enSection = section + 1241;  
      heSection = Mekoros.hebrew.encodeHebrewNumeral(section+1);
      heSection = heSection.slice(0,-1) + '"' + heSection.slice(-1);
    } else if (addressType === "Folio") {
      enSection = Mekoros.hebrew.intToFolio(section);  
      heSection = Mekoros.hebrew.encodeHebrewFolio(enSection);
    } else {
      enSection = section + 1;
      heSection = Mekoros.hebrew.encodeHebrewNumeral(section + 1);
    }
  ...
  titlesInText: function(text) {
    ...
    return Mekoros.books.filter(function(title) {
  ...
  wrapRefLinks: function(text) {
      ...
      const titles = Mekoros.titlesInText(text);
      if (titles.length === 0) {
          return text;
      }
      const refRe    = Mekoros.makeRefRe(titles);
  ...
  getText: function(ref, settings) {
    ...
    return this._ApiPromise(Mekoros.apiHost + this._textUrl(ref, settings))
        .then(d => {
            //swap out original versions from the server with the ones that Mekoros client side has sorted and updated with some fields.
            // This happens before saving the text to cache so that both caches are consistent
            if(d?.versions?.length){
                let versions = Mekoros._saveVersions(d.sectionRef, d.versions);
                d.versions = Mekoros._makeVersions(versions, false);
            }
            Mekoros._saveText(d, settings);
            return d;
        });
  },
  _bulkTexts: {},
  getBulkText: function(refs, asSizedString=false, minChar=null, maxChar=null, transLangPref=null) {
    ...
    const hostStr = `${Mekoros.apiHost}/api/bulktext/`;
  ...
  makeUrlForAPIV3Text: function(ref, requiredVersions, mergeText) {
    const host = Mekoros.apiHost;
    const endPoint = '/api/v3/texts/';
    const versions = requiredVersions.map(obj =>
        Mekoros.makeParamsStringForAPIV3(obj.language, obj.versionTitle)
    );
  ...
  getTextsFromAPIV3: async function(ref, requiredVersions, mergeText) {
    ...
    const url = Mekoros.makeUrlForAPIV3Text(ref, requiredVersions, mergeText);
    //TODO here's the place for getting it from cache
    const apiObject = await Mekoros._ApiPromise(url);
    //TODO here's the place for all changes we want to add, and saving in cache
    return apiObject;
  },
  getAllTranslationsWithText: async function(ref) {
    let returnObj = await Mekoros.getTextsFromAPIV3(ref, [{language: 'translation', versionTitle: 'all'}], false);
    return Mekoros._sortVersionsIntoBuckets(returnObj.versions);
  },
  _bulkSheets: {},
  getBulkSheets: function(sheetIds) {
    ...
    return this._cachedApiPromise({
      url: `${Mekoros.apiHost}/api/v2/sheets/bulk/${idStr}`,
      key: idStr,
  ...
  text: function(ref, settings = null, cb = null) {
    ...
    this._api(Mekoros.apiHost + this._textUrl(ref, settings), function(data) {
  ...
  translateISOLanguageCode(code, native = false) {
    ...
    return Mekoros.ISOMap?.[code.toLowerCase()]?.[lookupVar] || code;
  },
  getHebrewTitle: function(slug) {
    return Mekoros.ISOMap[slug] ? Mekoros.ISOMap[slug]["title"] ?  Mekoros.ISOMap[slug]["title"] : "Jewish Texts in " + Mekoros.ISOMap[slug]["name"] : "Jewish texts in " + slug ;
  },
  _versions: {},
  _translateVersions: {},
  ...
  getVersions: async function(ref) {
    ...
    if(!versionsInCache) {
        const url = Mekoros.apiHost + "/api/texts/versions/" + Mekoros.normRef(ref);
  ...
  getPortal: async function(portalSlug) {
      const cachedPortal = Mekoros._portals[portalSlug];
      if (cachedPortal) {
          return cachedPortal;
      }
      const response = await this._ApiPromise(`${Mekoros.apiHost}/api/portals/${portalSlug}`);
      Mekoros._portals[portalSlug] = response;
      return response;
  },
  subscribeMekorosNewsletter: async function(firstName, lastName, email, educatorCheck) {
      const payload = {
        language: Mekoros.interfaceLang === "hebrew" ? "he" : "en",
        educator: educatorCheck,
        firstName: firstName,
        lastName: lastName,
      };
      return await Mekoros.postToApi(`/api/subscribe/${email}`, null, payload);
  },
  subscribeSteinsaltzNewsletter: async function(firstName, lastName, email) {
      const payload = {firstName, lastName};
      return await Mekoros.postToApi(`/api/subscribe/steinsaltz/${email}`, null, payload);
  },
  ...
  subscribeMekorosAndSteinsaltzNewsletter: async function(firstName, lastName, email, educatorCheck) {
      const responses = await Promise.all([
          Mekoros.subscribeMekorosNewsletter(firstName, lastName, email, educatorCheck),
          Mekoros.subscribeSteinsaltzNewsletter(firstName, lastName, email),
      ]);
      return {status: "ok"};
  },
  ...
  getSourceVersions: async function(ref) {
     ...
     * @returns {string: [versions]} Versions by language
     */
    return Mekoros.getVersions(ref).then(versions => {
        return Mekoros.filterVersionsObjByLangs(versions, ['he'], true);
    });
  },
  getTranslations: async function(ref) {
     ...
     * @returns {string: [versions]} Versions by language
     */
    return Mekoros.getVersions(ref).then(result => {
        let versions = Mekoros.filterVersionsObjByLangs(result, ['he'], false);
        let heVersions = Mekoros.filterVersionsArrayByAttr(result?.he || [], {isSource: false});
  ...
  _saveVersions: function(ref, versions){
      for (let v of versions) {
        Mekoros._translateVersions[Mekoros.getTranslateVersionsKey(v.versionTitle, v.language)] = {
          en: v.versionTitle,
  ...
  setVersionPreference(sref, vtitle, lang) {
    ...
    const corpus = Mekoros.index(title).corpus;
    Mekoros.versionPreferences = Mekoros.versionPreferences.update(corpus, vtitle, lang);

    Mekoros.track.event("Reader", "Set Version Preference", `${corpus}|${vtitle}|${lang}`);
    Mekoros.editProfileAPI({version_preferences_by_corpus: {[corpus]: {[lang]: vtitle}}})
  },
  ...
  _textUrl: function(ref, settings) {
    ...
    let url = "/api/texts/" + Mekoros.normRef(ref);
  ...
  _getOrBuildTextData: function(key, settings) {
    ...
    const segmentData  = Mekoros.util.clone(this._texts[this._textKey(cached.ref, extend(settings, {context: 0}))]);
  ...
  _saveText: function(data, settings) {
    ...
    if (settings.context) {
      ...
      let newData         = Mekoros.util.clone(data);
      ...
      const newSettings   = Mekoros.util.clone(settings);
    ...
    if (data.isSpanning) {
      const spanningContextSettings = Mekoros.util.clone(settings);
      ...
      for (let i = 0; i < data.spanningRefs.length; i++) {
        ...
        Mekoros.getText(data.spanningRefs[i], spanningContextSettings)
      }
    }
  },
  ...
  _splitTextSection: function(data, settings) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      const segment_data = Mekoros.util.clone(data);
      const sectionRef =isSuperSection ? data.ref + delim + (i+1): data.sectionRef
      extend(segment_data, {
        ref: ref,
        heRef: data.heRef + delim + Mekoros.hebrew.encodeHebrewNumeral(i+start),
        text: en[i],
        he: he[i],
  ...
  _splitSpanningText: function(data) {
    ...
    for (var i = 0; i < length; i++) {
      var section        = Mekoros.util.clone(data);
      section.text       = en[i];
      section.he         = he[i];
    }
  },
  ...
  _wrapRefs: function(data) {
    ...
    if (typeof data.text === "string") {
      data.text = Mekoros.wrapRefLinks(data.text);
    } else {
      data.text = data.text.map(Mekoros.wrapRefLinks);
    }
    return data;
  }, */
  ...
  _cacheFromToc: function(tocBranch, parentsPath = "", parentsOrders = [], rewrittenFrom = "", rewrittenTo = "") {
    ...
    for (let i = 0; i < tocBranch.length; i++) {
      ...
      if (tocBranch[i].searchRoot) {
          ...
          Mekoros._tocOrderLookup[rewrittenTo] = thisOrder;
      } else if (rewrittenFrom) {
          const new_path = thisPath.replace(RegExp("^" + rewrittenFrom), rewrittenTo);
          Mekoros._tocOrderLookup[new_path] = thisOrder;
      } else {
          Mekoros._tocOrderLookup[thisPath] = thisOrder;
      }

      if ("category" in tocBranch[i]) {
          Mekoros._translateTerms[tocBranch[i].category] = {"en": tocBranch[i].category, "he": tocBranch[i].heCategory};
          if (tocBranch[i].contents) {
              Mekoros._cacheFromToc(tocBranch[i].contents, thisPath, thisOrder, rewrittenFrom,  rewrittenTo)
          }
      } else {
          Mekoros.index(tocBranch[i].title, tocBranch[i]);
      }
    }
  },
  compareSearchCatPaths: function(a,b) {
      ...
      const aPath = Mekoros._tocOrderLookup[a];
      const bPath = Mekoros._tocOrderLookup[b];
  ...
  getIndexDetails: function(title) {
    return this._cachedApiPromise({
        url:   Mekoros.apiHost + "/api/v2/index/" + title + "?with_content_counts=1&with_related_topics=1",
        key:   title,
  ...
  postSegment: function(ref, versionTitle, language, text, success, error) {
    ...
    this.getName(ref, true)
        .then(data => {
            ...
            $.ajax({
              dataType: "json",
              url: Mekoros.apiHost + "/api/texts/" + data.url,
              data: d,
              type: "POST",
  ...
  getRef: function(ref) {
    ...
    return Mekoros.getText(ref, {context: 1});
  },
  ref: function(ref, callback) {
      if (callback) {
          throw new Error("Use of Mekoros.ref() with a callback has been deprecated in favor of Mekoros.getRef()");
      }
  ...
  openTransBannerApplies: (book, textLanguage) => {
      ...
      const currCorpus = Mekoros.index(book)?.corpus;
  ...
  lexiconCompletion: function(word, lexicon, callback) {
      ...
      return $.ajax({
          dataType: "json",
          url: Mekoros.apiHost + "/api/words/completion/" + word + (lexicon ? "/" + lexicon : ""),
  ...
  getTopicCompletions: function (word, callback) {
       return this._cachedApiPromise({
          url: `${Mekoros.apiHost}/api/topic/completion/${word}`, key: word,
          store: Mekoros._topicCompletions,
          processor: callback
  ...
  getLexiconWords: function(words, ref) {
    ...
    let url = Mekoros.apiHost + "/api/words/" + encodeURIComponent(words)+"?always_consonants=1&never_split=1" + (ref?("&lookup_ref="+encodeURIComponent(ref)):"");
  ...
  getLinks: function(ref) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (ref in this._links) {
            ...
            let url = Mekoros.apiHost + "/api/links/" + ref + "?with_text=0";
  ...
  _cacheTranslationsOfEssays: function(links) {
    for (let link of links) {
      ...
      Mekoros._translateTerms[link.displayedText.en] = link.displayedText;
    }
  },
  ...
  _saveItemsByRef: function(data, store) {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      var refs = "anchorRefExpanded" in data[i] ? data[i].anchorRefExpanded : Mekoros.splitRangingRef(ref);
  ...
  _filterLinks: function(links, filter) {
    ...
    var index           = Mekoros.index(filter);
    ...
    return links.filter(function(link){
      ...
      if (isEssay) { return link.type === "essay" && Mekoros.util.inArray(link.displayedText["en"], filter) !== -1; }

      return (Mekoros.util.inArray(link.category, filter) !== -1 ||
              Mekoros.util.inArray(link["collectiveTitle"]["en"], filter) !== -1 );
    });
  },
  ...
  hasEssayLinks: function(ref) {
      let links = [];
      ref.map(function(r) {
          const newlinks = Mekoros.getLinksFromCache(r);
  ...
  essayLinks: function(ref, versions) {
    ...
    return essayLinks.sort((a, b) => Mekoros.refContains(a["sourceRef"], b["sourceRef"]));
  }
  ,
  _linkSummaries: {},
  linkSummary: function(ref, excludedSheet) {
    ...
    const oref          = (typeof ref == "string") ? Mekoros.ref(ref) : Mekoros.ref(ref[0]);
    ...
    const normRef = Mekoros.humanRef(ref);
    ...
    for (let i = 0; i < links.length; i++) {
      ...
      if (link["collectiveTitle"]["en"] in category.books) {
        ...
        category.books[link["collectiveTitle"]["en"]].categoryList = Mekoros.index(link["index_title"]) ? Mekoros.index(link["index_title"]).categories :[]
      } else {
    ...
    const oRef       = Mekoros.getRefFromCache(baseRef);
    const sectionRef = oRef ? oRef.sectionRef : baseRef;
    if (ref !== sectionRef) {
      const sectionLinks = Mekoros.getLinksFromCache(sectionRef);
      for (let i = 0; i < sectionLinks.length; i++) {
        ...
        if (l.category === "Commentary") {
          ...
          if (!(l["collectiveTitle"]["en"] in summary["Commentary"].books)) {
            ...
            summary["Commentary"].books[l["collectiveTitle"]["en"]].categoryList = Mekoros.index(l["index_title"]) ? Mekoros.index(l["index_title"]).categories :[]
          }
        }
      }
    }
    ...
    const summaryList = Object.keys(summary).map(function(category) {
      ...
      categoryData.books = Object.keys(categoryData.books).map(function(book) {
        ...
        const fullTitleIndex = Mekoros.index(bookData.fullTitle) ? Mekoros.index(bookData.fullTitle) : index
        ...
        if (bookData.categoryList != "Quoting Commentary") {
            ...
            // bookData.enShortDesc = Mekoros.tocItemsByCategories(bookData.categoryList).map((e)=>(e.category || e.title === bookData.book ? e.enShortDesc: null))
        }
        if (bookData.categoryList && !bookData.enShortDesc) {
            const desc = Mekoros.getDescriptionDict(bookData.book, bookData.categoryList)
            if (desc) {
      ...
      categoryData.books.sort(Mekoros.linkSummaryBookSort.bind(null, cat));

      return categoryData;
    });
    // Sort the categories
    const categoryOrder = Mekoros.toc.map(function(cat) { return cat.category; });
    ...
    Mekoros._linkSummaries[cacheKey] = summaryList;
    return summaryList;
  },
  ...
  linkSummaryBookSortHebrew: function(category, a, b) {
    return Mekoros.linkSummaryBookSort(category, a, b, true);
  },
  commentarySectionRef: function(commentator, baseRef) {
    ...
    var links = Mekoros.getLinksFromCache(baseRef);
    links = Mekoros._filterLinks(links, [commentator]);
    if (!links || !links.length || links[0].isSheet) { return null; }

    var pRefs = links.map(link => Mekoros.parseRef(link.sourceRef));
    ...
    const sorter = (a, b) => {
      return a.match(/\d+[ab]/) ?
        Mekoros.hebrew.dafToInt(a) - Mekoros.hebrew.dafToInt(b)
    ...
    var ref = Mekoros.humanRef(Mekoros.makeRef(commentaryRef));

    return ref;
  },
    _descDict: {}, // cache for the description dictionary
      getDescriptions: function(keyName, categoryList) {
      const catlist = Mekoros.tocItemsByCategories(categoryList)
        ...
        let descs = d[keyName] || d[Mekoros.index(keyName).collectiveTitle] || d[keyName.split(" on")[0]];
    ...
    getDescriptionDict: function(keyName, categoryList){
        ...
        if (Object.keys(this._descDict).length === 0){
            ...
            Mekoros.toc.map(e=> {this._descDict[[e.category, []]] = [e.enShortDesc, e.heShortDesc]})
  ...
  privateNotes: function(refs, callback) {
    ...
    if (typeof refs == "string") {
      ...
      var notesByRef = refs.map(function(ref) {
        return Mekoros._privateNotes[ref];
      });
    ...
    if (notes) {
      ...
      var aggregateCallback = function() {
       ...
       if (Mekoros.privateNotesLoaded(refs) && callback) {
        callback(Mekoros.privateNotes(refs));
       }
      };
      refs.map(function(ref) {
       if (ref in Mekoros._privateNotes) { return; } // Only make API calls for unloaded refs
       var url = Mekoros.apiHost + "/api/notes/" + Mekoros.normRef(ref) + "?private=1";
  ...
  privateNotesLoaded: function(refs) {
    ...
    refs.map(function(ref) {
      if (!(ref in Mekoros._privateNotes)) {
        return false;
      }
    });
    return true;
  ...
  allPrivateNotes: function(callback) {
    ...
    if (this._allPrivateNotes) {
      ...
      const url = Mekoros.apiHost + "/api/notes/all?private=1";
      this._api(url, (data) => {
        if ("error" in data) {
          return;
        }
  ...
  notesTotalCount: function(refs) {
    ...
    var notes = Mekoros.notes(refs) || [];
    if (Mekoros._uid) {
      var myNotes  = Mekoros.privateNotes(refs) || [];
      notes = notes.filter(function(note) { return note.owner !== Mekoros._uid }).concat(myNotes);
    }
    return notes.length;
  },
    ...
    return new Promise((resolve, reject) => {
      $.ajax({
        ...
        success: () => {
          ...
          Mekoros.track.event("Tools", "Delete Note", noteId);
          resolve();
        },
        error: reject
      });
...
_media: {},
  mediaByRef: function(refs) {
    refs = typeof refs == "string" ? Mekoros.splitRangingRef(refs) : refs.slice();
    var ref = Mekoros.normRefList(refs);
  ...
  webPagesByRef: function(refs) {
    ...
    webpages.map(page => page.isHebrew = Mekoros.hebrew.isHebrew(page.title));

    webpages = webpages.sort((a, b) => {
      // Sort first by page language matching interface language
      if (a.isHebrew !== b.isHebrew) { return (b.isHebrew ? -1 : 1) * (Mekoros.interfaceLang === "hebrew" ? -1 : 1); }
  ...
  topicsByRefCount: function(refs) {
    const topics = Mekoros.topicsByRef(refs);
 ...
 guidesByRef: function(refs) {
  ...
  relatedApi: function(ref, callback) {
    var url = Mekoros.apiHost + "/api/related/" + Mekoros.normRef(ref) + "?with_sheet_links=1";
    return this._api(url, data => {
      if ("error" in data) {
        return;
      }
      var originalData = Mekoros.util.clone(data);
  ...
  relatedPrivate: function(ref, callback) {
    ...
    if (ref in this._relatedPrivate) {
      callback(this._relatedPrivate[ref]);
       ...
       var url = Mekoros.apiHost + "/api/related/" + Mekoros.normRef(ref) + "?private=1";
       this._api(url, function(data) {
          if ("error" in data) {
            return;
          }
          ...
          let potentialEmptyRefs = Mekoros.splitRangingRef(ref);
  ...
  repairCaseVariant: function(query, data) {
    if (Mekoros.isACaseVariant(query, data)) {
      ...
      for (var n = 0; n < topLength; n++) {
        ...
        for (var i = 0; i < length; i++) {
          ...
          var section = typeof startSection == "string" ?
                        Mekoros.hebrew.intToDaf(n+Mekoros.hebrew.dafToInt(startSection))
                        : n + startSection;
          var number  = i + start;
  ...
  stripImagesFromSegments: function(segments) {
      // Used by sheets editors.  Mekoros.makeSegments creates a list of segments and this function handles the images.
      return segments.map(x => {
          x.he = Mekoros.util.stripImgs(x.he);
          x.en = Mekoros.util.stripImgs(x.en);
          return x;
      })
  },
  ...
  isCommentaryWithBaseText(book) {
       ...
       * @param {Object} book: Corresponds to a book in Mekoros.toc
       */
  ...
  isCommentaryRefWithBaseText(ref) {
      ...
      const parsedRef = Mekoros.parseRef(refToCheck);
      const book = Mekoros.index(parsedRef.index);
      ...
      return Mekoros.isCommentaryRefValid(book, parsedRef);
  },
  isCommentaryRefValid(book, parsedRef) {
       ...
       * this can be tested by modifying the parsedRef and then calling Mekoros.parseRef on the modified parsedRef
       * @param {Object} parsedRef: Object created with Mekoros.parseRef.  Its `ref` property is what we need to check
       * @param {Object} book: Object created with Mekoros.index.  We want to use `book`'s metadata to modify the ref.
       */
      const parsedRefCopy = Object.create(parsedRef);  // copy object to avoid modifying Mekoros._parseRef
      const baseText = book.base_text_titles[0];
      parsedRefCopy.ref = parsedRefCopy.ref.replace(book.title, baseText);
      return (!Mekoros.parseRef(parsedRefCopy.ref).error);
  },
  convertCommentaryRefToBaseRef(commRef) {
    ...
    const commRefCopy = Object.create(commRef);  // need to create a copy so that the Mekoros._parseRef cache isn't changed
    if (commRefCopy.sections.length <= 2 || !many_to_one) {
        ...
        return Mekoros.humanRef(commRefCopy.ref);
    }
    else if (many_to_one) {
        ...
        return Mekoros.humanRef(Mekoros.makeRef(commRefCopy));
    }
    return Mekoros.humanRef(commRef.ref);
  },
  getBaseRefAndFilter(ref) {
    ...
    if (Array.isArray(ref)) {
        const parsedRefs = ref.map(x => Mekoros.parseRef(x)); // get a parsed ref version of `ref` in order to access book's collective title, base_text_titles, and base_text_mapping
        book = Mekoros.index(parsedRefs[0].index);
        ref = parsedRefs.map(x => Mekoros.convertCommentaryRefToBaseRef(x));
    }
    else {
        const parsedRef = Mekoros.parseRef(ref); // get a parsed ref version of `ref` in order to access book's collective title, base_text_titles, and base_text_mapping
        book = Mekoros.index(parsedRef.index);
        ref = Mekoros.convertCommentaryRefToBaseRef(parsedRef);
    }
  ...
  commentaryList: function(title, toc) {
    ...
    /** Returns the list of commentaries for 'title' which are found in Mekoros.toc **/
    var toc = arguments.length <= 1 || arguments[1] === undefined ? Mekoros.util.clone(Mekoros.toc) : arguments[1];
    if (title != null){
    ...
    for (var i=0; i < toc.length; i++) {
        ...
        if (curTocElem.title) { //this is a book
          ...
          results = results.concat(Mekoros.commentaryList(title, curTocElem.contents));
        }
    }
    return results;
  },
  ...
  tocItemsByCategories: function(cats) {
    ...
    const object = Mekoros.tocObjectByCategories(cats);
    return object ? Mekoros.util.clone(object.contents) : [];
  },
  ...
  getPassages: function(refs) {
    ...
    return this._ApiPromise(Mekoros.apiHost + "/api/passages/" + refs.join("|"));
  },
  ...
  getSavedItem: ({ ref, versions }) => {
    return Mekoros.saved.items.find(s => s.ref === ref && Mekoros.areVersionsEqual(s.versions, versions));
  },
  removeSavedItem: ({ ref, versions }) => {
    Mekoros.saved.items = Mekoros.saved.items.filter(x => !(x.ref === ref && Mekoros.areVersionsEqual(versions, x.versions)));
  },
  toggleSavedItem: ({ ref, versions, sheet_owner, sheet_title }) => {
    return new Promise((resolve, reject) => {
      const action = !!Mekoros.getSavedItem({ ref, versions }) ? "delete_saved" : "add_saved";
      const savedItem = { ref, versions, time_stamp: Mekoros.util.epoch_time(), action, sheet_owner, sheet_title };
      if (Mekoros._uid) {
        $.post(`${Mekoros.apiHost}/api/profile/sync?no_return=1`,
          ...
          if (!!response['error']) {
            ...
            if (action === "add_saved" && !!response.created && response.created.length > 0) {
              Mekoros.saved.items = response.created.concat(Mekoros.saved.items);
            } else {
              // delete
              Mekoros.removeSavedItem({ ref, versions });
            }
            resolve(response);
          }
  ...
  editProfileAPI: (partialProfile) => {
    ...
    return new Promise((resolve, reject) => {
      $.post(`${Mekoros.apiHost}/api/profile`, data, resolve);
    });
  },
  followAPI: (slug, ftype) => {
    return Mekoros._ApiPromise(Mekoros.apiHost + `/api/profile/${slug}/${ftype}`);
  },
  messageAPI: (uid, message) => {
    const data = {json: JSON.stringify({recipient: uid, message: message.escapeHtml()})};
    return new Promise((resolve, reject) => {
      $.post(`${Mekoros.apiHost}/api/messages`, data, resolve);
    });
  },
  chatMessageAPI: (roomId, senderId, timestamp, messageContent) => {
    ...
    return new Promise((resolve, reject) => {
      $.post(`${Mekoros.apiHost}/api/chat-messages`, data, resolve);
    })
  },
  getChatMessagesAPI: (roomId) => {
    return Mekoros._ApiPromise(Mekoros.apiHost + `/api/chat-messages/?room_id=${roomId}`);
  },
  getRefSavedHistory: tref => {
    return Mekoros._ApiPromise(Mekoros.apiHost + `/api/user_history/saved?tref=${tref}`);
  },
  _profiles: {},
  profileAPI: slug => {
    return Mekoros._cachedApiPromise({
      url:   Mekoros.apiHost + "/api/profile/" + slug,
      key:   slug,
      store: Mekoros._profiles
    });
  },
  userHistory: {loaded: false, items: []},
  loadUserHistory: function (limit, callback) {
      const skip = Mekoros.userHistory.items.length;
      const url = `/api/profile/user_history?secondary=0&annotate=1&limit=${limit}&skip=${skip}`;
      fetch(url)
          .then(response => response.json())
          .then(data => {
              Mekoros.userHistory.loaded = true;
              Mekoros.userHistory.items.push(...data);
              callback();
          });
  },
  saveUserHistory: function(history_item) {
    ...
    if(!Mekoros.is_history_enabled || !history_item) {
        return;
    }
    const history_item_array = Array.isArray(history_item) ? history_item : [history_item];
    for (let h of history_item_array) {
      h.time_stamp = Mekoros.util.epoch_time();
    }
    if (Mekoros._uid) {
        $.post(Mekoros.apiHost + "/api/profile/sync?no_return=1&annotate=1",
              {user_history: JSON.stringify(history_item_array)},
              data => {
                // Insert new items to beginning of history
                Mekoros.userHistory.items = data.created.concat(Mekoros.userHistory.items);
              } );
    } else {
      ...
      Promise.all(history_item_array.filter(x=>!x.secondary).map(h => new Promise((resolve, reject) => {
        Mekoros.getRef(h.ref).then(oref => {
          h.he_ref = oref.heRef;
          resolve(h);
        });
      }))).then(new_hist_array => {
        ...
        Mekoros.userHistory.items = new_hist_array.concat(user_history);

        //console.log("saving history cookie", new_hist_array);
        if (Mekoros._inBrowser) {
    ...
    Mekoros.last_place = history_item_array.filter(x=>!x.secondary).concat(Mekoros.last_place);  // while technically we should remove dup. books, this list is only used on client
  },
    isNewVisitor: () => {
        return (
  ...
  uploadProfilePhoto: (formData) => {
    return new Promise((resolve, reject) => {
      if (Mekoros._uid) {
        $.ajax({
          url: Mekoros.apiHost + "/api/profile/upload-photo",
          type: 'post',
          data: formData,
          contentType: false,
          processData: false,
  ...
  sortTopicsCompareFn: function(a, b) {
    ...
    const [aDisplayOrder, bDisplayOrder] = [a, b].map(x => Mekoros.isTopicTopLevel(x.slug) ? 10000 : x.displayOrder);
  ...
  bookSearchPathFilterAPI: title => {
    return Mekoros._cachedApiPromise({
      url:   Mekoros.apiHost + "/api/search-path-filter/" + title,
      key:   title,
      store: Mekoros._bookSearchPathFilter
    });
  },
  _topics: {},
  ...
  getTopic: function(slug, {annotated=true, with_html=false}={}) {
    const cat = Mekoros.topicTocCategory(slug);
    ...
    if (!!cat && !!Mekoros._CAT_REF_LINK_TYPE_FILTER_MAP[cat.slug]) {
      ref_link_type_filters = Mekoros._CAT_REF_LINK_TYPE_FILTER_MAP[cat.slug];
    }
    const a = 0 + annotated;
    ...
    for (let [linkTypeSlug, linkTypeObj] of Object.entries(data.refs)) {
      for (let refObj of linkTypeObj.refs) {
        ...
        if (!tabs[tabKey]) {
          ...
          if (tabKey === 'sheets') {
            title = {en: 'Sheets', he: Mekoros._('Sheets')};
          }
          if (tabKey === 'sources') {
            title = {en: 'Sources', he: Mekoros._('Sources')};
          }
          tabs[tabKey] = {
            refMap: {},
            title,
  ...
  slugsToTitles: function() {
    ...
    if (!Mekoros._topicSlugsToTitles) { this._topicSlugsToTitles = Mekoros.topic_toc.reduce(Mekoros._initTopicTocSlugToTitleReducer, {});}
    return Mekoros._topicSlugsToTitles;
  },
  _topicTocPages: null,
  ...
  _initTopicTocReducer: function(a,c) {
    ...
    a[Mekoros._topicTocPageKey(c.slug)] = c.children;
    for (let sub_c of c.children) {
      Mekoros._initTopicTocReducer(a, sub_c);
    }
    return a;
  },
  _initTopicTocSlugToTitleReducer: function(a,c) {
    ...
    for (let sub_c of c.children) {
      Mekoros._initTopicTocSlugToTitleReducer(a, sub_c);
    }
    return a;
  },
  ...
  _initTopicTocCategoryReducer: function(a,c) {
    ...
    for (let sub_c of c.children) {
      ...
      Mekoros._initTopicTocCategoryReducer(a, sub_c);
    }
    return a;
  },
  ...
  isTopicTopLevel: function(slug) {
    ...
    return Mekoros.topic_toc.filter(x => x.slug == slug).length > 0;
  },
  sheets: {
    _loadSheetByID: {},
    ...
    deleteSheetById: function(id) {
      return Mekoros._ApiPromise(`/api/sheets/${id}/delete`);
    },
    _userSheets: {},
    userSheets: function(uid, callback, sortBy="date", offset=0, numberToRetrieve=0) {
      ...
      if (sheets) {
        ...
        const url = Mekoros.apiHost + "/api/sheets/user/" + uid + "/" + sortBy + "/" + numberToRetrieve + "/" + offset;
        Mekoros._ApiPromise(url).then(data => {
    ...
    publicSheets: function(offset, limit, options, skipCache, callback) {
      ...
      if (sheets && !skipCache) {
        ...
        const url = Mekoros.apiHost + "/api/sheets/all-sheets/" + path

        Mekoros._api(url, function(data) {
    ...
    sheetsByRef: function(ref, cb) {
      ...
      if (typeof ref == "string") {
        ...
        var sheets = [];
        ref.map(function(r) {
          var newSheets = Mekoros.sheets.sheetsByRef(r);
          if (newSheets) {
    ...
    _saveSheetsByRefData: function(ref, data) {
      ...
      return Mekoros._saveItemsByRef(data, this._sheetsByRef);
    },
    _userSheetsByRef: {},
    userSheetsByRef: function(ref, cb) {
      ...
      if (typeof ref == "string") {
        ...
        var sheets = [];
        ref.map(function(r) {
          var newSheets = Mekoros.sheets.userSheetsByRef(r);
          if (newSheets) {
            sheets = sheets.concat(newSheets);
          }
        });
      ...
      if (sheets) {
        ...
        Mekoros.relatedPrivate(ref, function(data) {
          if (cb) { cb(data.sheets); }
        });
      }
      return sheets;
    ...
    _saveUserSheetsByRefData: function(ref, data) {
      ...
      return Mekoros._saveItemsByRef(data, this._userSheetsByRef);
    },
    sheetsTotalCount: function(refs) {
      ...
      var sheets = Mekoros.sheets.sheetsByRef(refs) || [];
      if (Mekoros._uid) {
        var mySheets = Mekoros.sheets.userSheetsByRef(refs) || [];
        sheets = sheets.filter(function(sheet) { return sheet.owner !== Mekoros._uid }).concat(mySheets);
      }
  ...
  getTranslation: function(key) {
    const url = Mekoros.apiHost + "/api/texts/translations/" + key;
    const store = this._translations;
    return this._cachedApiPromise({url, key, store})
  },
  _collections: {},
  getCollection: function(key) {
      const url = Mekoros.apiHost + "/api/collections/" + encodeURIComponent(key);
      const store = this._collections;
      return this._cachedApiPromise({url, key, store});
  },
  getCollectionFromCache: function(key) {
    return Mekoros._collections[key];
  },
  _collectionsList: {},
  getCollectionsList: function() {
      return this._cachedApiPromise({
        url: Mekoros.apiHost + "/api/collections",
        key: "list",
        store: Mekoros._collectionsList
      });
  },
  getCollectionsListFromCache() {
    return Mekoros._collectionsList.list;
  },
  _userCollections: {},
  getUserCollections: function(uid) {
    return this._cachedApiPromise({
      url: `${Mekoros.apiHost}/api/collections/user-collections/${uid}`,
      key: uid,
      store: Mekoros._userCollections
    });
  },
  getUserCollectionsFromCache(uid) {
    return Mekoros._userCollections[uid];
  },
  _userCollectionsForSheet: {},
  getUserCollectionsForSheet: function(sheetID) {
    return this._cachedApiPromise({
      url: `${Mekoros.apiHost}/api/collections/for-sheet/${sheetID}`,
      key: sheetID,
      store: Mekoros._userCollectionsForSheet
    });
  },
  getUserCollectionsForSheetFromCache(sheetID) {
    return Mekoros._userCollectionsForSheet[sheetID];
  },
  getBackgroundData() {
    return Mekoros._ApiPromise("/api/background-data?locale=" + Mekoros.interfaceLang)
      .then(data => { Mekoros = extend(Mekoros, data); });
  },
  calendarRef: function(calendarTitle) {
    const cal = Mekoros.calendars.filter(cal => cal.title.en === calendarTitle);
    return cal.length ? cal[0].ref : null;
  },
  _translateTerms: {},
  _cacheHebrewTerms: function(terms) {
      Mekoros._translateTerms = extend(terms, Mekoros._translateTerms);
  },
  hebrewTerm: function(name) {
    ...
    if (name in Mekoros._translateTerms) {
        return Mekoros._translateTerms[name]["he"];
    } else if (Mekoros._translateVersions[Mekoros.getTranslateVersionsKey(name, 'en')]) {
        return Mekoros._translateVersions[Mekoros.getTranslateVersionsKey(name, 'en')]["he"];
    } else if (Mekoros._translateVersions[Mekoros.getTranslateVersionsKey(name, 'he')]) {
        return Mekoros._translateVersions[Mekoros.getTranslateVersionsKey(name, 'he')]["he"];
    } else if (name in categories) {
        return  categories[name];
    } else if (Mekoros.index(name)) {
        return Mekoros.index(name).heTitle;
    } else {
        return name;
    }
  },
  hebrewTranslation: function(inputStr, context = null){
    ...
    if (context && context in Mekoros._i18nInterfaceStringsWithContext){
      translatedString = Mekoros._getStringCaseInsensitive(Mekoros._i18nInterfaceStringsWithContext[context], inputStr);
      if (translatedString !== null) return translatedString;
    }
    if ((translatedString = Mekoros._getStringCaseInsensitive(Mekoros._i18nInterfaceStrings, inputStr)) !== null ) {
      return translatedString;
    }
    if ((translatedString = Mekoros.hebrewTerm(inputStr)) != inputStr) {
      return translatedString;
    }
    if (inputStr.indexOf(" | ") !== -1) {
      var inputStrs = inputStr.split(" | ");
      return Mekoros._(inputStrs[0])+ " | " + Mekoros._(inputStrs[1]);
    } else {
  ...
  _: function(inputStr, context=null){
    if(Mekoros.interfaceLang != "english"){
      return Mekoros.translation(Mekoros.interfaceLang, inputStr, context);
    } else {
      return inputStr;
    }
  },
  ...
  _r: function (inputRef) {
    const oref = Mekoros.getRefFromCache(inputRef);
    if (!oref) { return inputRef; }
    return Mekoros.interfaceLang != "english" ? oref.heRef : oref.ref;
  },
  ...
  _cacheSiteInterfaceStrings: function() {
    ...
    if (!Mekoros._siteSettings) { return; }
    ["SITE_NAME", "LIBRARY_NAME"].map(key => {
      Mekoros._i18nInterfaceStrings[Mekoros._siteSettings[key]["en"]] = Mekoros._siteSettings[key]["he"];
    });
  },
  ...
  incrementalPromise: async (fetchResponse, data, increment, setResponse, setCancel) => {
    ...
    */
    ...
    while (lastEndIndex <= data.length) {
      ...
      const { promise, cancel } = Mekoros.makeCancelable(fetchResponse(tempData));
      setCancel(cancel);
...
Mekoros.unpackDataFromProps = function(props) {
  ...
  for (let i = 0; i < initialPanels.length; i++) {
      ...
      if (panel.text) {
        ...
        if(panel.text?.versions?.length){
            let versions = Mekoros._saveVersions(panel.text.sectionRef, panel.text.versions);
            panel.text.versions = Mekoros._makeVersions(versions, false);
        }

        Mekoros._saveText(panel.text, settings);
      }
      if(panel.bookRef){
         if(panel.versions?.length){
            let versions = Mekoros._saveVersions(panel.bookRef, panel.versions);
            panel.versions = Mekoros._makeVersions(versions, false);
         }
      }
      if (panel.indexDetails) {
        Mekoros._indexDetails[panel.bookRef] = panel.indexDetails;
      }
      if (panel.sheet) {
        Mekoros.sheets._loadSheetByID[panel.sheet.id] = panel.sheet;
      }
  }
  if (props.collectionData) {
    Mekoros._collections[props.initialCollectionSlug] = props.collectionData;
  }
  if (props.translationsData) {
    Mekoros._translations[props.initialTranslationsSlug] = props.translationsData;
  }
  if (props.topicData) {
    Mekoros._topics[props.initialTopic] = Mekoros.processTopicsData(props.topicData);
  }
  if (props.topicList) {
    Mekoros._topicList = props.topicList;
  }
  if (props.collectionListing) {
      Mekoros._collectionsList.list = props.collectionListing;
  }
  Mekoros.versionPreferences = new VersionPreferences(props.versionPrefsByCorpus);
  Mekoros.util._initialPath = props.initialPath ? props.initialPath : "/";
  Mekoros.unpackBaseProps(props);

  Mekoros.getBackgroundData();
};

Mekoros.unpackBaseProps = function(props){
  ...
  for (const element of dataPassedAsProps) {
      if (element in props) {
        Mekoros[element] = props[element];
      }
  }
}

Mekoros.loadServerData = function(data){
...
Mekoros.util    = Util;
Mekoros.hebrew  = Hebrew;
Mekoros.track   = Track;
Mekoros.palette = palette;

Mekoros.palette.indexColor = function(title) {
      return title && Mekoros.index(title) ?
          Mekoros.index(title)['primary_category'] ?
              Mekoros.palette.categoryColor(Mekoros.index(title)['primary_category']):
                Mekoros.palette.categoryColor(Mekoros.index(title).categories[0]):
          Mekoros.palette.categoryColor("Other");
};
Mekoros.palette.refColor = ref => Mekoros.palette.indexColor(Mekoros.parseRef(ref).index);

Mekoros = extend(Mekoros, Strings);

Mekoros.setup = function(data, props = null) {
    Mekoros.loadServerData(data);
    let baseProps = props !=null ? props : (typeof DJANGO_VARS === "undefined" ? undefined : DJANGO_VARS.props);
    Mekoros.unpackBaseProps(baseProps);
    Mekoros.util.setupPrototypes();
    Mekoros.util.setupMisc();
    var cookie = Mekoros.util.handleUserCookie(Mekoros._uid);
    // And store current uid in analytics id
    Mekoros._analytics_uid = Mekoros._uid;
    Mekoros._makeBooksDict();
    Mekoros._cacheFromToc(Mekoros.toc);
    Mekoros._cacheHebrewTerms(Mekoros.terms);
    Mekoros._cacheSiteInterfaceStrings();
    //console.log(`sending user logged in status to GA, uid as bool: ${!!Mekoros._uid} | analytics id: ${Mekoros._analytics_uid}`);
    Mekoros.track.setUserData(!!Mekoros._uid, Mekoros._analytics_uid);
    Mekoros.search = new Search(Mekoros.searchIndexText, Mekoros.searchIndexSheet);
};
Mekoros.setup();


export default Mekoros;
