...
class Search {
    constructor(searchIndexText, searchIndexSheet) {
      ...
      this._cache = {};
      this.mekorosQueryQueue = {hits: {hits:[], total: new SearchTotal(), max_score: 0.0}, lastSeen: -1};
      this.dictaQueryQueue = {lastSeen: -1, hits: {total: new SearchTotal(), hits:[]}};
      this.queryDictaFlag = true;
      this.dictaCounts = null;
      this.mekorosSheetsResult = null;
      this.buckets = [];
      this.queryAborter = new HackyQueryAborter();
      this.dictaSearchUrl = 'https://mekoros.loadbalancer.dicta.org.il';
    }
    cache(key, result) {
        if (result !== undefined) {
    ...
    mekorosQuery(args, isQueryStart, wrapper) {
        return new Promise((resolve, reject) => {
            const jsonData = this.sortedJSON(this.get_query_object(args));
            const cacheKey = "mekorosQuery|" + jsonData;
            ...
            wrapper.addQuery($.ajax({
                url: `${Mekoros.apiHost}/api/search-wrapper/es8`,
                type: 'POST',
                data: jsonData,
            ...
            if (args.type === "sheet") {
                this.mekorosSheetsResult = x;
                return null;
            }
            for (let i=0; i < x.hits.hits.length; i++) {
                x.hits.hits[i]['comp_date'] = x.hits.hits[i]._source.comp_date;
                x.hits.hits[i]['cameFrom'] = 'Mekoros';
                x.hits.hits[i]['score'] =  x.hits.hits[i]['_score'] * -1;
            }

            let mergedHits = this.mekorosQueryQueue.hits.hits.concat(x.hits.hits);
            let lastSeen = this.mekorosQueryQueue.lastSeen + x.hits.hits.length;
            this.mekorosQueryQueue = x;
            this.mekorosQueryQueue.hits.hits = mergedHits;
            this.mekorosQueryQueue.lastSeen = lastSeen;
        });

    }
    /**
     * Function to reformat the Hebrew Ref from the Dicta convention - תנ״ר/נביאים/ספר זכריה/פרק א/פסוק א
     * to the Mekoros Hebrew Ref convention (the results of Ref('Zechariah 1.1').he_normal())
     * @param {*} ref - the incoming Dicta Ref
     * @returns cleanedHebrewRef - according to Mekoros conventions
     */
    ...
    isDictaQuery(args) {
        return Mekoros.hebrew.isHebrew(args.query) && !args.exact;
    }
    sortedJSON(obj) {
    ...
    mergeQueries(addAggregations, sortType, filters) {
        ...
        if(addAggregations) {
            ...
            let newBuckets = this.mekorosQueryQueue['aggregations']['path']['buckets'].filter(
        ...
        if (!!filters.length) {
            ...
            result.hits.total = this.mekorosQueryQueue.hits.total.combine(this.dictaQueryQueue.hits.total);
        }

        let mekorosHits = (this.queryDictaFlag)
            ? this.mekorosQueryQueue.hits.hits.filter(i => !(i._source.categories.includes("Tanakh")))
            : this.mekorosQueryQueue.hits.hits;
        let dictaHits = this.dictaQueryQueue.hits.hits;

        let finalHits;
        if (!(dictaHits.length) || !(mekorosHits.length)) /* either or both queues are empty */ {
            finalHits = dictaHits.concat(mekorosHits).sort((i, j) => i[sortType] - j[sortType]);
            this.mekorosQueryQueue.hits.hits = [];
            this.dictaQueryQueue.hits.hits = [];
        }
        else {
            // when sorting by relevance adjust Dicta score's mean and standard deviation to match Mekoros's
            if (sortType === "score"){
                let mekorosMeanScore = mekorosHits.reduce(
                    (total, nextValue) => total + nextValue.score / mekorosHits.length, 0
                );
                let dictaMeanScore = dictaHits.reduce(
                    (total, nextValue) => total + nextValue.score / mekorosHits.length, 0
                );

                let mekorosStd = Math.sqrt(mekorosHits.reduce(
                    (total, nextValue) => total + Math.pow(nextValue.score - mekorosMeanScore, 2), 0
                ));
                let dictaStd = Math.sqrt(dictaHits.reduce(
                    (total, nextValue) => total + Math.pow(nextValue.score - dictaMeanScore, 2), 0
                ));

                let factor = (dictaStd !== 0) ?mekorosStd/dictaStd : 1;
                ...
                let delta = mekorosMeanScore - dictaMeanScore;
                for (let i=0; i < dictaHits.length; i++) {
    ...
    execute_query(args) {
        ...
        if (this.queryDictaFlag) {
            Promise.all([
                this.mekorosQuery(args, updateAggreagations, queryAborter),
                this.dictaQuery(args, updateAggreagations, queryAborter),
                this.dictaBooksQuery(args, queryAborter)
            ]).then(() => {
                if (args.type === "sheet") {
                    this._cacheQuery(args, this.mekorosSheetsResult);
                    args.success(this.mekorosSheetsResult);
                }
                else {
            ...
            this.mekorosQuery(args, updateAggreagations, queryAborter)
                .then(() => {
                    if (args.type === "sheet") {
                        ...
                        this._cacheQuery(args, this.mekorosQueryQueue);
                        args.success(this.mekorosQueryQueue);
                    }
                })
        }
    ...
    _cacheQuery(args, results) {
        const cacheKey = this._queryCacheKey(args);
        results = Mekoros.util.clone(results);
    ...
    buildFilterTree(aggregation_buckets, appliedFilters) {
        ...
        filters = filters.filter(x => Mekoros._tocOrderLookup[x.aggKey]);

        // sort into search toc tree order
        filters.sort((a,b) => Mekoros.compareSearchCatPaths(a.aggKey, b.aggKey));
        ...
        for (let i = 0; i < filters.length; i++) {
            ...
            for (let k = j; k < f.path.length; k++) {
                const node = new FilterNode({
                        ...
                        "heTitle": Mekoros.hebrewTerm(f.path[k]),
    ...
    buildAndApplySheetFilters(aggregation_buckets, appliedFilters, appliedFilterAggTypes, aggType) {
      const availableFilters = aggregation_buckets.map( b => {
        const isHeb = Mekoros.hebrew.isHebrew(b.key);
        const enTitle = isHeb ? '' : b.key;
        const heTitle = isHeb ? b.key : (aggType === 'collections' || !Mekoros.terms[b.key] ? '' : Mekoros.terms[b.key].he);
