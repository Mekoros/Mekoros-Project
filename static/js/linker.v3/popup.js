...
export class PopupManager {
    ...
    setupPopup() {
        ...
        this.popUpElem.id = "mekoros-popup";
        ...
        html += '<style scoped>' +
            ...
            '}'+
            'a.mekoros-ref-debug.mekoros-link-failed{'+
                'border: 3px solid red' +
            '}'+
            'a.mekoros-ref-debug.mekoros-link-ambiguous{'+
        ...
        html += '<div id="mekoros-linker-header">' +
            ...
            '<div class="mekoros-footer">' +
                ...
                '</span>' : "") +
                (this.debug ?
                    '<span class="mekoros-read-more-button" id="mekoros-report-btn"><a>Report</a></span>'
                    : ''
                ) +
            '</div>';
        ...
        let draggie = new Draggabilly(this.popUpElem, {handle: "#mekoros-linker-header"});

        this.linkerHeader = this.popUpElem.querySelector("#mekoros-linker-header");
        this.linkerFooter = this.popUpElem.querySelector(".mekoros-footer");
        this.textBox = this.popUpElem.querySelector(".mekoros-text");
        this.heTitle = this.popUpElem.querySelector("#mekoros-title .he");
        this.enTitle = this.popUpElem.querySelector("#mekoros-title .en");
        this.heElems = this.popUpElem.querySelectorAll(".he");
        this.enElems = this.popUpElem.querySelectorAll(".en");

        if (this.mode === "popup-click") {
            this.popUpElem.querySelector('#mekoros-close').addEventListener('click', this.hidePopup, false);
    ...
    showPopup(elem, {ref, heRef, en=[], he=[], primaryCategory, isTruncated=false}) {
        ...
        if (isTruncated) {
            ...
            truncated.innerHTML = this.interfaceLang === "english" ? "Read the complete text on Mekoros" : "לקריאת הטקסט המלא בספריא";
        ...
        if (this.mode === "popup-click") {
            [].forEach.call(this.popUpElem.querySelectorAll(".mekoros-popup-ref"), function(link) {link.setAttribute('href', elem.href);});
        ...
        if (this.debug) {
            const reportBtn = document.querySelector('#mekoros-report-btn');
    ...
    bindEventHandler(elem, baseUrl, source) {
        ...
        const url = `${baseUrl}/${source.url}?lang=${urlLang}&utm_source=${utmSource}&utm_medium=mekoros_linker`;
