...
django.setup()
import argparse
from mekoros.system.cloudflare import MekorosCloudflareManager
from mekoros.utils.util import get_directory_content
from datetime import datetime
try:
    from mekoros.settings import USE_CLOUDFLARE
...
if __name__ == '__main__':
    ...
    if args.files:
        if USE_CLOUDFLARE:
            MekorosCloudflareManager().purge_batch_cloudflare_urls(args.files)
        else:
        ...
        if USE_CLOUDFLARE:
            MekorosCloudflareManager().purge_static_files_from_cloudflare(timestamp=args.timestamp)
        else:
        ...
        if USE_CLOUDFLARE:
            MekorosCloudflareManager().purge_static_files_from_cloudflare()
        else:
