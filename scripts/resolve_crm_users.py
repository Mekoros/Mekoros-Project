...
def find_matching_and_update(row, dry_run):
    ...
    if matching_mongo_profile:
        ...
        if profile.email == row['Mekoros App Email']:
            row['email matches'] = True
        try:
            if dry_run:
                row['updated'] = "dry_run"
            else:
                row['updated'] = CrmInfoStore.save_crm_id(row['Mekoros App User: ID'], profile.email, "SALESFORCE", profile)
                CrmInfoStore.mark_sustainer(profile, is_sustainer(row))
        except:
            row['updated'] = False
    else:
        row['nb_id matches'] = False
        profile = UserProfile(email=row['Mekoros App Email'])
        row['mongo_id'] = profile.id
        if profile.email == 'test@mekoros.com':
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        ...
        description="Resolves CRM Users to add Mekoros app user ids (salesforce) based on Nationbuilder ID or Email"
    )
    ...
    with open(args.file[0], "r") as sf_inf, \
        ...
        fieldnames = ["Contact: NationBuilder Id", "Contact: Contact ID", "Contact: First Name", 'Contact: Last Name',
                      'Mekoros App User: Mekoros App User Name', 'Mekoros App User: ID', "Mekoros App Email",
        ...
        for index, row_r in enumerate(csv_reader):
            ...
            if index % 1000 == 0:  # in case script gets dropped
                print(f'Row {index+last_skipped_line+2}: {row_r["Mekoros App User: ID"]}')
