# -*- coding: utf-8 -*-
import django
django.setup()

from collections import defaultdict

from mekoros.system.database import db
from mekoros.model import *

authors          = defaultdict(int)
public_authors   = defaultdict(int)
authors_by_views = defaultdict(int)

sheets = db.sheets.find()

for sheet in sheets:

    owner = sheet.get("owner", 0)
    profile = UserProfile(id=owner)

    if "interface_language" in profile.settings and profile.settings["interface_language"] == "hebrew":
        authors[owner] += 1
        authors_by_views[owner] += sheet.get("views", 0)
        if "status" in sheet and sheet["status"] == "public":
            public_authors[owner] += 1

sorted_authors          = sorted(iter(authors.items()), key=lambda x: -x[1])
sorted_public_authors   = sorted(iter(public_authors.items()), key=lambda x: -x[1])
sorted_authors_by_views = sorted(iter(authors_by_views.items()), key=lambda x: -x[1])


print("\n\nTop Public Sheet Authors")
for author in sorted_public_authors[:20]:
    profile = UserProfile(id=author[0])
    print("%s: %d public sheets - www.mekoros.com/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))

print("\n\nTop Total Sheet Authors")
for author in sorted_authors[:20]:
    profile = UserProfile(id=author[0])
    print("%s: %d sheets - www.mekoros.com/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))

print("\n\nTop Total Sheet Authors by Views")
for author in sorted_authors_by_views[:20]:
    profile = UserProfile(id=author[0])
    print("%s: %d total sheet views - www.mekoros.com/profile/%s - %s" % (profile.full_name, author[1], profile.slug, profile.email))
